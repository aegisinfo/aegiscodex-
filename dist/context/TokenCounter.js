...
export class TokenCounter {
     ...
     *
     */
    static countTokens(messages, modelName) {
     ...
     *
     */
    static countMessageTokens(message, modelName) {
        return this.countTokens([message], modelName);
    }
    /**
     *
     */
    static countTextTokens(text, modelName) {
        const encoding = this.getEncoding(modelName);
     ...
     *
     */
    static countToolCallTokens(toolCalls, encoding) {
        let tokens = 0;
     ...
     *
     */
    static shouldCompact(messages, modelName, maxTokens, thresholdPercent = 0.8) {
            ...
            try {
                ...
                try {
                    ...
                    const fallbackEncoding = {
                        encode: (text) => new Array(Math.ceil(text.length / 4)),
                        decode: () => '',
                        free: () => { },
                    };
     ...
     *
     */
    static getRemainingTokens(messages, modelName, maxContextTokens, maxOutputTokens = 4096) {
        const currentTokens = this.countTokens(messages, modelName);
     ...
     *
     */
    static truncateMessages(messages, modelName, maxTokens) {
        const result = [];
        let currentTokens = 0;
        // 
