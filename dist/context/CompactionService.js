...
export class CompactionService {
    /** 压缩阈值百分比（80%） */
    static THRESHOLD_PERCENT = 0.8;
    /** 保留比例（20%） */
    static RETAIN_PERCENT = 0.2;
    /** 降级时保留比例（30%） */
    static FALLBACK_RETAIN_PERCENT = 0.3;
    /**
     *
     */
    static shouldCompact(messages, modelName, maxContextTokens) {
        return TokenCounter.shouldCompact(messages, modelName, maxContextTokens, this.THRESHOLD_PERCENT);
    }
    /**
     *
     */
    static async compact(messages, options) {
        const preTokens = options.actualPreTokens
            ?? TokenCounter.countTokens(messages, options.modelName);
        // 执行 Compaction Hook - 检查是否应该阻止压
        const shouldPrevent = await onCompaction(preTokens, messages.length, options.sessionId || 'unknown', options.projectDir || process.cwd());
        if (shouldPrevent) {
     ...
     *
     */
    static async generateSummary(messages, fileContents, options) {
        const prompt = this.buildCompactionPrompt(messages, fileContents);
        // 如果提供了 chatService，使用它来生成总
        if (options.chatService && typeof options.chatService.chat === 'function') {
            try {
                const response = await options.chatService.chat([
     ...
     *
     */
    static buildCompactionPrompt(messages, fileContents) {
        // 格式化消息历
        ...
        // 格式化文件内
        const filesText = fileContents.map(file => `### ${file.path}\n\`\`\`\n${file.content}\n\`\`\``).join('\n\n');
...
Keep the summary concise but comprehensive. Focus on information that would be needed to continue the work.`;
     ...
     *
     */
    static createFallbackSummary(messages) {
...
The conversation can continue normally.`;
     ...
     *
     */
    static filterOrphanToolMessages(messages) {
        // 收集保留消息中所有 tool_call 
     ...
     *
     */
    static createSummaryMessage(id, summary) {
        return {
            role: 'user',
     ...
     *
     */
    static fallbackCompact(messages, options, preTokens, error) {
        ...
        const summaryMessage = this.createSummaryMessage(nanoid(), `[Automatic compaction failed; using fallback]
...
The conversation can continue, but consider retrying compaction later with /compact.`);
     ...
     *
     */
    static async forceCompact(messages, options) {
        return this.compact(messages, { ...options, trigger: 'auto' });
    }
}
//# sourceMappingURL=CompactionService.js.map