...
function extractTags(content) {
    ...
    ['docker', 'railway', 'flask', 'python', 'typescript', 'react', 'aegis', 'joke',
        ...
        .forEach(t => { if (content.toLowerCase().includes(t))
        tags.push(t); });
    return tags;
}
...
function extractEntities(content) {
    ...
    if (funcMatches)
        entities.push(...funcMatches.map(m => m.split(/\s+/)[1]));
    // File paths
    const fileMatches = content.match(/(?:`[^`]+`)/g);
    if (fileMatches)
        entities.push(...fileMatches.map(m => m.replace(/`/g, '')));
    // Package/module names
    const pkgMatches = content.match(/(?:from\s+|require\s*\(\s*['"])['"]([^'"]+)['"]/g);
    if (pkgMatches)
        entities.push(...pkgMatches.map(m => {
            const inner = m.match(/['"]([^'"]+)['"]/);
...
function scoreImportance(content, role) {
    let score = role === 'user' ? 0.7 : 0.4;
    if (/\?/.test(content))
        score += 0.1;
    if (/```|function|class|import|export/.test(content))
        score += 0.15;
    if (/decided|conclusion|solution|fixed|resolved|summary|key/i.test(content))
        score += 0.15;
    ...
    if (/^decision:|^conclusion:|^important:/i.test(content.trim()))
        score += 0.2;
...
export class SharedMemory {
    ...
    // ── Add entry ───────────────────────────────────────────────────────────
    async add(content, source, session, tags = [], role = 'assistant', immediate = false) {
        await this.ensureReady();
        if (!this.isWriteAllowed(session))
            return null;
    ...
    `, [
        ...
        // Enforce max entries
        const count = this.db.exec(`SELECT COUNT(*) AS c FROM memories`)[0]?.values[0][0] ?? 0;
        if (count > max) {
            this.db.run(`
    ...
    async summarizeAndStoreSession(sessionId, apiKey, baseURL, model) {
        await this.ensureReady();
        const cfg = getMemoryConfig();
        if (cfg.summaryEnabled === false)
            return false;
        const rows = this.db.exec(`SELECT * FROM memories WHERE session = ? AND summary = 0 ORDER BY timestamp DESC LIMIT 100`, [sessionId]);
        const sessionEntries = this.rowsToEntries(rows);
        if (sessionEntries.length < 3)
            return false;
        const existing = this.db.exec(`SELECT id FROM memories WHERE session = ? AND summary = 1 LIMIT 1`, [sessionId]);
    ...
    async summarizeSession(sessionId, entries, apiKey, baseURL, model) {
        ...
        try {
            ...
            const res = await fetch(isAnthropic ? 'https://api.anthropic.com/v1/messages' : `${baseURL}/chat/completions`, { method: 'POST', headers, body: JSON.stringify(body) });
            const data = await res.json();
            return isAnthropic
                ? data?.content?.[0]?.text
    ...
    // ── buildContext ─────────────────────────────────────────────────────────
    async buildContext(query, maxEntries = 4, currentSession) {
        await this.ensureReady();
        ...
        if (subscribed) {
            ...
            const summaryRows = this.db.exec(`SELECT * FROM memories WHERE summary = 1 ORDER BY timestamp DESC LIMIT 3`);
        ...
        if (currentSession) {
            const sessionRows = this.db.exec(`SELECT * FROM memories WHERE session = ? AND summary = 0 ORDER BY timestamp DESC LIMIT 4`, [currentSession]);
     ...
     */
    _reminderShownThisProcess = false;
    getUpgradeReminder(sessionId) {
        if (this._reminderShownThisProcess)
            return null;
        if (this.isSubscribed())
            return null;
        if (this.isWriteAllowed(sessionId))
            return null; // still in free tier
    ...
    searchByTopic(topic, limit = 10) {
        const rows = this.db.exec(`SELECT * FROM memories WHERE topics LIKE ? ORDER BY importance DESC, timestamp DESC LIMIT ?`, [`%"${topic}"%`, limit]);
        return this.rowsToEntries(rows);
    }
    /** Search by entity */
    searchByEntity(entity, limit = 10) {
        const rows = this.db.exec(`SELECT * FROM memories WHERE entities LIKE ? ORDER BY importance DESC, timestamp DESC LIMIT ?`, [`%"${entity}"%`, limit]);
        return this.rowsToEntries(rows);
    }
}
