...
function ollamaQueryNeedsTools(messages) {
...
export class OpenAIChatService {
    ...
    async chat(messages, tools, signal, streamCallbacks, attempt = 0) {
        ...
        try {
            ...
            const cleanContent = (s) => s.replace(/[\u200b-\u200f\u2028-\u202f\ufeff]/g, '');
            ...
            const isTransient = (error instanceof OpenAI.APIError && [429, 500, 502, 503, 504].includes(error.status)) || (error instanceof Error && (error.message.includes('Connection error') ||
            ...
            if (error instanceof OpenAI.APIError) {
                if (isOllama && error.status === 400 && error.message.includes('does not support tools')) {
                    throw new Error(`Model "${this.model}" does not support tools.\n\n` +
