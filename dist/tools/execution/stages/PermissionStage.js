...
import { PermissionMode, PermissionResult, } from '../types.js';
...
export class PermissionStage {
    sessionApprovals;
    name = 'permission';
    permissionChecker;
    defaultMode;
    constructor(config, sessionApprovals, defaultMode = PermissionMode.DEFAULT) {
    ...
    async process(execution) {
        ...
        try {
            ...
            execution.abort(`Parameter validation failed: ${error instanceof Error ? error.message : String(error)}`);
            return;
        }
        // 2. 构建权限签
     ...
     *
     */
    applyModeOverrides(toolKind, checkResult, permissionMode) {
        // 1. YOLO 模式：批准所有（最高优先
    ...
    checkSensitiveFiles(execution) {
        ...
        if (sensitiveFiles.length === 0)
            return;
        // 高敏感文件直接拒
        const highSensitive = sensitiveFiles.filter(f => f.result.level === SensitivityLevel.HIGH);
     ...
     *
     */
    async executePermissionHook(execution, toolName) {
        return onPermissionRequest(toolName, execution.params, execution.context.sessionId || 'unknown', execution.context.workspaceRoot || process.cwd(), execution.context.permissionMode);
    }
}
//# sourceMappingURL=PermissionStage.js.map