 ...
 * 2. 检测工具列表冲突
 */
export const validatePermissions = (argv) => {
    // 1. 处理 --yolo 快捷方
    if (argv.yolo) {
        if (argv.permissionMode && argv.permissionMode !== 'yolo') {
            throw new Error('Cannot use both --yolo and --permission-mode with different values');
        }
 ...
 * 3. 验证会话选项
 */
export const loadConfiguration = async (argv) => {
    // 跳过 --init 命令的配置加
    if (argv.init) {
        return;
    }
 ...
 * 1. 验证输出格式组合
 */
export const validateOutput = (argv) => {
    // 验证输出格式只能与 --print 一起使
