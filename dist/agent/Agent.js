...
import { ExecutionPipeline, createToolRegistry, getBuiltinTools, PermissionMode, } from '../tools/index.js';
...
export class Agent {
    ...
    async initialize() {
        ...
        try {
            ...
            throw new Error(`Agent 初始化失败: ${error instanceof Error ? error.message : '未知错误'}`);
        }
    }
    // ========== 公共方
     ...
     *
     */
    async executeLoop(message, context, options) {
        // === 1. 准备阶
        // 构建消息历
        const messages = [];
        // 添加系统提
        ...
        while (true) {
            ...
            let turnResult;
            try {
                turnResult = await this.chatService.chat(messages, tools.length > 0 ? tools : undefined, options?.signal, streamCallbacks);
            }
            catch (error) {
                // 处理中
            ...
            if (!turnResult.toolCalls || turnResult.toolCalls.length === 0) {
                ...
                const shouldContinue = await onStop('end_turn', context.sessionId || 'unknown', process.cwd());
                if (shouldContinue) {
                ...
                if (turnResult.content && turnResult.content.length > 50) {
                    ...
                    if (context.sessionId) {
                        ...
                        sharedMemory.summarizeAndStoreSession(context.sessionId, this.config.apiKey, this.config.baseURL, this.config.model).catch(() => { });
                    }
                }
                return {
                    success: true,
     ...
     *
     */
    detectIncompleteIntent(content) {
        if (!content)
            return false;
    ...
    async executeToolCall(toolCall, context) {
        ...
        // 通过执行管道执行工
        const result = await this.executionPipeline.execute(toolCall.function.name, params, pipelineContext);
            ...
            if (mcpTools.length > 0) {
                for (const tool of mcpTools) {
                    try {
                        ...
                        agentDebug.warn(`注册 MCP 工具 "${tool.name}" 失败:`, error.message);
                    }
                }
                ...
                try {
                    ...
                    for (const name of currentNames) {
                        if (!freshNames.has(name))
                            this.toolRegistry.unregisterMcpTool(name);
                    }
                    ...
                    agentDebug.warn('MCP hot-reload failed:', err.message);
                }
            });
        }
        catch (error) {
            agentDebug.warn('MCP 工具加载失败:', error.message);
