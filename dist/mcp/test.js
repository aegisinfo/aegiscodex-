...
import { McpRegistry, McpConnectionStatus, ErrorType, DEFAULT_HEALTH_CHECK_CONFIG, DEFAULT_CONNECTION_CONFIG, } from './index.js';
import { createMcpTool } from './createMcpTool.js';
const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
// 模拟 MCP 工具定义（用于测试 Schema 转
const MOCK_TOOL_DEFINITION = {
    name: 'test_tool',
...
async function runTests() {
    ...
    if (McpConnectionStatus.DISCONNECTED === 'disconnected' &&
    ...
    // 测试 ErrorType 枚
    if (ErrorType.NETWORK_TEMPORARY === 'network_temporary' &&
    ...
    // 测试默认配
    if (DEFAULT_HEALTH_CHECK_CONFIG.intervalMs === 30000 &&
    ...
    try {
        const tool = createMcpTool(mockClient, 'mock-server', MOCK_TOOL_DEFINITION, 'test_tool');
    ...
    try {
        ...
        if (listTool) {
            ...
            try {
                const result = await listTool.execute({ path: __dirname }, {});
                if (result.success) {
                ...
                console.log(`  ⚠️  工具调用异常: ${err.message}`);
            }
        }
        else {
