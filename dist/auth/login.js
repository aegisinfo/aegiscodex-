...
export async function runLogin() {
    return new Promise((resolve, reject) => {
        ...
        server.listen(0, '127.0.0.1', () => {
            ...
            const timer = setTimeout(() => done(new Error('Login timed out after 5 minutes. Please try again.')), LOGIN_TIMEOUT_MS);
...
export async function runLoginPassword() {
...
function saveClaudeCodeOAuthToken(token) {
    let lines = [];
    try {
...
export async function runLoginClaudePro() {
...
export function runLogout() {
    ...
    try {
        fs.writeFileSync(CONFIG_FILE, JSON.stringify(cfg, null, 2));
        process.stdout.write(hadKey
            ? `\n${C.green}✓ Logged out.${C.reset}\n\n`
            : `\n${C.muted}(no active session)${C.reset}\n\n`);
    }
    catch (e) {
        process.stderr.write(`Failed to update config: ${e.message}\n`);
        process.exit(1);
    }
}
