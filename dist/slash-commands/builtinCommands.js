...
export const modelCommand = {
    ...
    async handler(args, context) {
        ...
        if (subcommand === 'add') {
            ...
            const newModel = { id, name: name || id, model, baseURL, apiKey: apiKey || '', provider: 'openai-compatible' };
        ...
        if (subcommand === 'remove' || subcommand === 'rm') {
            const id = parts[1];
            if (!id)
                return { success: false, type: 'error', content: 'usage: /model remove <id>' };
            if (!models.find(m => m.id === id)) {
        ...
        if (subcommand === 'list') {
            if (models.length === 0)
                return { success: true, type: 'info', content: 'no models configured.' };
            const lines = models.map(m => `${m.id === currentModelId ? '▶' : ' '} \`${m.id}\` ${m.name || ''} (${m.model || ''})`);
            ...
            if (targetModel) {
                ...
                try {
                    ...
                    cfg.default = {
                        ...
                        baseURL: targetModel.baseURL || targetModel.baseUrl || cfg.default?.baseURL,
                        apiKey: targetModel.apiKey || cfg.default?.apiKey,
                    };
                    fs.writeFileSync(cfgPath, JSON.stringify(cfg, null, 2));
                }
                catch { /* non-fatal */ }
                return { success: true, type: 'success', message: `model -> ${targetModel.label || targetModel.name || targetModel.model || targetModel.id}` };
            }
...
function detectTaskType(task) {
    ...
    if (/^(build|create|new|scaffold|generate|make|init|bootstrap)\b/.test(lower))
        return 'scaffold';
    if (/^(refactor|restructure|reorganize|redesign|rewrite)\b/.test(lower))
        return 'refactor';
    if (/^(review|audit|inspect|check|analyze|security|vulnerab)\b/.test(lower))
        return 'review';
    return 'default';
}
/**
    ...
    async handler(args) {
        ...
        if (args?.startsWith('load ')) {
            ...
            try {
                ...
                let existing = [];
                try {
                    existing = JSON.parse(fs.readFileSync(memFile, 'utf8'));
                }
                catch { }
                const newEntries = (Array.isArray(convs) ? convs : convs.conversations || []).map((c) => ({
        ...
        if (args?.trim() === 'clear') {
            if (!subscribed)
                return { success: false, type: 'error', error: 'Not subscribed' };
            sharedMemory.clear();
        ...
        if (mem.activatedAt)
            lines.push(`  Activated: ${mem.activatedAt.slice(0, 10)}`);
        if (recent.length > 0) {
            lines.push('  Recent:');
            recent.slice(-3).reverse().forEach((e) => {
...
const councilCommand = {
    ...
    async handler(args) {
        const question = args?.trim();
        if (!question)
            return { success: false, type: 'error', error: 'Usage: /council <question>' };
...
const cloudCommand = {
    ...
    async handler(args) {
        ...
        if (trimmed.startsWith('key ')) {
            ...
            if (apiKey.length < 16)
                return { success: false, type: 'error', error: 'API key too short' };
...
const appCommands = BUILTIN_APPS.map(createAppCommand);
// ─── Export all commands ───────────────────────────────────────────
