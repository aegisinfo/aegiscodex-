 ...
 * - .claude/commands/
 */
export async function initializeCustomCommands(workspaceRoot) {
 ...
 *
 */
function convertCustomToSlashCommand(customCmd, registry) {
 ...
 *
 */
export async function reloadCustomCommands(workspaceRoot) {
    // 移除现有的自定义命
 ...
 *
 */
export function registerSlashCommand(command) {
    commandRegistry[command.name] = command;
}
/**
 *
 */
export function unregisterSlashCommand(name) {
 ...
 *
 */
export function getFuzzyCommandSuggestions(input) {
    ...
    // 精确前缀匹配优
    const prefixMatches = searchableCommands.filter((cmd) => cmd.name.startsWith(query) || cmd.aliases.some((a) => a.startsWith(query)));
 ...
 *
 */
export function getCommandCompletions(partialInput) {
