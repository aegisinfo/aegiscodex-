...
async function callOpenAICompatible(baseUrl, apiKey, model, question, persona) {
...
let _provider = null;
function callPrimary(q, p) { if (!_provider)
    _provider = pickProvider(); return callOpenAICompatible(_provider.baseUrl, _provider.apiKey, _provider.model, q, p); }
function callDeepSeek(q, p) { return callOpenAICompatible('https://api.deepseek.com/v1', process.env.DEEPSEEK_API_KEY || '', 'deepseek-chat', q, p); }
