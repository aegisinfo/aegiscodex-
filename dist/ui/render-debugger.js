 ...
 */
function patchReactCreateElement(React) {
 ...
 */
function patchStoreSubscribe(store) {
    if (!store || storeUnsubPatched)
        return () => { };
    storeUnsubPatched = true;
    const origSubscribe = store.subscribe.bind(store);
    const wrappedSubscribe = (selector, callback) => {
...
function analyzeRenderEvents() {
    ...
    for (let i = 1; i < allEvents.length; i++) {
        ...
        if (interval < CASCADE_THRESHOLD && allEvents[i].component !== allEvents[i - 1].component) {
            result.cascades.push(`${allEvents[i - 1].component} → ${allEvents[i].component} (${interval.toFixed(1)}ms)`);
        }
    }
    // 3. Infinite render loop detection (same component in rapid succession)
    let loopStreak = 0;
    for (let i = 1; i < allEvents.length; i++) {
        if (allEvents[i].component === allEvents[i - 1].component &&
 ...
 */
export async function startRenderDebugger(options) {
    if (state.enabled) {
    ...
    if (verbose)
        console.log('   Verbose mode: every render event will be logged');
    // Patch RAF
    const unpatches = [];
    ...
    globalThis.__RENDER_DEBUGGER__ = {
        state,
        unpatches,
 ...
 */
export function stopRenderDebugger() {
    if (!state.enabled) {
        return 'Render debugger was not running.';
    }
    state.enabled = false;
    // Restore patches
    const debug = globalThis.__RENDER_DEBUGGER__;
    if (debug?.unpatches) {
    ...
    delete globalThis.__RENDER_DEBUGGER__;
 ...
 */
export function getRenderEvents() {
    return [...state.events];
}
/**
 * Get the top N most frequently rendered components.
 */
export function getHottestComponents(n = 10) {
    const counts = {};
    for (const ev of state.events) {
        if (ev.type === 'mount' || ev.type === 'update') {
...
//# sourceMappingURL=render-debugger.js.map