...
// Store
import { useInitializationStatus, useActiveModal, useSessionId, usePendingCommands, sessionActions, commandActions, getState, subscribe, } from '../../store/index.js';
...
const QueuedCommands = React.memo(() => {
    ...
    if (pendingCommands.length === 0)
        return null;
    return (_jsxs(Box, { flexDirection: "column", marginTop: 1, children: [_jsxs(Text, { color: theme.colors.text.muted, dimColor: true, children: ['\u2500\u2500', " Queued (", pendingCommands.length, ") ", '\u2500\u2500'] }), pendingCommands.map((cmd, index) => (_jsx(Box, { marginLeft: 2, children: _jsxs(Text, { color: theme.colors.text.muted, dimColor: true, children: [index + 1, ". ", cmd.length > 60 ? cmd.slice(0, 57) + '...' : cmd] }) }, index)))] }));
});
...
export const AegisInterface = ({ apiKey, baseURL, model, initialMessage, debug, resumeSessionId, }) => {
                ...
                if (found) {
                    ...
                    if (agentRef.current) {
                        import('../../agent/Agent.js').then(({ Agent }) => {
                            ...
                            Agent.create({
                                apiKey,
                                baseURL: found.baseURL || found.baseUrl,
                                model: displayName,
                            }).then(agent => {
    ...
    // ==================== Render ====================
    if (isInitializing) {
        return (_jsx(Box, { flexDirection: "column", padding: 1, children: _jsxs(Box, { children: [_jsx(Text, { color: "yellow", children: _jsx(Spinner, { type: "dots" }) }), _jsx(Text, { color: "yellow", children: " Initializing..." })] }) }));
    }
    if (initError) {
    ...
    if (initializationStatus === 'needsSetup') {
        return (_jsxs(Box, { flexDirection: "column", padding: 1, children: [_jsx(Text, { color: "yellow", children: "No models configured." }), _jsx(Text, { color: "gray", children: "Please configure a model in ~/.aegis/config.json" })] }));
    }
    if (selectorState.isVisible) {
        return (_jsxs(Box, { flexDirection: "column", width: "100%", children: [_jsx(Box, { flexDirection: "column", marginBottom: 1, children: _jsxs(Box, { children: [_jsx(Text, { color: theme.colors.text.muted, children: '\u250C\u2500' }), _jsx(Text, { bold: true, color: theme.colors.primary, children: " aegis" }), _jsx(Text, { color: theme.colors.text.secondary, children: "code " })] }) }), _jsx(RecentMessagesPreview, { terminalWidth: terminalWidth - 2, count: 3 }), _jsx(InteractiveSelector, { title: selectorState.title, options: selectorState.options, onSelect: handleSelectorSelect, onCancel: handleSelectorCancel, focusId: FocusId.SELECTOR })] }));
    }
    ...
    if (messageCount === 0 && !hasPendingInitialMessage) {
        return (_jsxs(Box, { flexDirection: "column", width: "100%", height: terminalHeight, children: [_jsx(Box, { flexGrow: 1 }), _jsx(WelcomeMessage, { terminalWidth: terminalWidth - 2 }), _jsx(InputArea, { onSubmit: handleSubmit }), _jsx(ChatStatusBar, { model: currentModel }), isExiting && exitSessionId && _jsx(ExitMessage, { sessionId: exitSessionId })] }));
    }
    ...
    return (_jsxs(Box, { flexDirection: "column", width: "100%", children: [_jsx(MessageList, { terminalWidth: terminalWidth - 2 }), _jsx(QueuedCommands, {}), confirmationState.isVisible && confirmationState.details && (_jsx(ConfirmationPrompt, { details: confirmationState.details, onResponse: handleResponse })), _jsx(InputArea, { onSubmit: handleSubmit }), _jsx(ChatStatusBar, { model: currentModel }), isExiting && exitSessionId && (_jsx(ExitMessage, { sessionId: exitSessionId }))] }));
};
