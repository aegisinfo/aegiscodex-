...
function estimateContentLines(text, terminalWidth) {
    if (!text)
        return 0;
...
function estimateMessageLines(content, thinking, terminalWidth, contentBlocks) {
...
export const MessageList = React.memo(({ terminalWidth, terminalHeight, onScrolledUpChange, onRenderLatency, pendingCommandCount = 0, }) => {
    ...
    const messageLines = useMemo(() => completedMessages.map(msg => estimateMessageLines(msg.content, msg.thinking, terminalWidth, msg.contentBlocks)), [completedMessages, terminalWidth]);
    ...
    const streamingLines = streamingMsg && buffer
        ? estimateMessageLines(streamingMsg.content + buffer.content, (streamingMsg.thinking || '') + buffer.thinking, terminalWidth, streamingMsg.contentBlocks)
        : 0;
    ...
    const startMsgIndex = useMemo(() => {
        ...
        while (lo < hi) {
            ...
            if (lineOffsets[mid] <= clampedLineOffset)
                lo = mid;
            else
                hi = mid - 1;
        }
    ...
    function ensureRafRunning() {
        ...
        const flushTick = (now) => {
            ...
            const liveStreaming = liveStoreState.session.messages.find(m => m.isStreaming && isActiveStreamingMessage(m));
    ...
    // ==================== Render ====================
    return (_jsxs(Box, { flexDirection: "column", children: [useWindowing && !isAtBottom && !streamingMsg && (_jsx(Box, { children: _jsxs(Text, { color: themeManager.getTheme().colors.text.muted, dimColor: true, children: ["\u2191 ", clampedLineOffset, " lines above"] }) })), visibleMessages.map((msg) => (_jsx(MessageRenderer, { content: msg.content, role: msg.role, terminalWidth: terminalWidth, showPrefix: true, thinking: msg.thinking, isStreaming: false, showAllThinking: showAllThinking, contentBlocks: msg.contentBlocks }, msg.id))), streamingMsg && (() => {
                ...
                return (_jsx(MessageRenderer, { content: combinedContent, role: streamingMsg.role, terminalWidth: terminalWidth, showPrefix: true, thinking: combinedThinking, isStreaming: true, showAllThinking: showAllThinking, contentBlocks: mergedBlocks }, streamingMsg.id));
            })(), showLastMsgOutside && (_jsx(Box, { children: _jsx(Text, { color: themeManager.getTheme().colors.text.muted, dimColor: true, children: "\u22EF latest (End to jump here) \u22EF" }) })), showLastMsgOutside && (_jsx(MessageRenderer, { content: completedMessages[completedMessages.length - 1].content, role: completedMessages[completedMessages.length - 1].role, terminalWidth: terminalWidth, showPrefix: true, thinking: completedMessages[completedMessages.length - 1].thinking, isStreaming: false, showAllThinking: showAllThinking, contentBlocks: completedMessages[completedMessages.length - 1].contentBlocks }, 'last-' + completedMessages[completedMessages.length - 1].id))] }));
});
