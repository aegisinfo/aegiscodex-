...
import { sessionActions, getState, subscribe } from '../../store/index.js';
async function initHooks(sessionId) {
    try {
...
export function useAgent(options) {
    ...
    useEffect(() => {
        const initAgent = async () => {
            if (getState().app.initializationStatus === 'needsSetup')
                return;
            try {
                if (debugRef.current) {
                ...
                if (resumeSessionId) {
                    ...
                    if (loaded) {
                        ...
                        const restoredMessages = contextMessages
