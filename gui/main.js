...
// node-pty lives in app.asar.unpacked when packaged — require it once here
// so both spawnPty and spawnShell share the same resolved module.
function loadPty() {
  if (app.isPackaged) {
    ...
    try { return require(unpackedPty); } catch {}
  }
  try { return require("@homebridge/node-pty-prebuilt-multiarch"); } catch {}
  return null;
}
...
function findNode() {
  ...
  // Bundled Node binary (packed into resources/bin/ by electron-builder)
  const bundled = app.isPackaged
    ? path.join(process.resourcesPath, "bin", isWin ? "node.exe" : "node")
    : null;

  const candidates = isWin
    ? [bundled, "node.exe"].filter(Boolean)
    : [
        bundled,
  ...
  for (const c of candidates) {
    try {
      const ver = require("child_process")
        .execFileSync(c, ["--version"], { encoding: "utf8" }).trim();
