...
install_linux() {
  ...
  local deb="$tmp/aegis-gui.deb"
  if command -v curl &>/dev/null; then
    curl -fsSL --progress-bar "$R2/aegis-gui.deb" -o "$deb"
  else
    wget -q --show-progress "$R2/aegis-gui.deb" -O "$deb"
  fi
  ...
  echo ""
  echo "  Launch:  ags"
  echo "  Or:      AEGIS Code  (application menu)"
  echo ""
  echo "  On first launch, go to Settings and add your API key."
  echo "  aegiscloud.org"
}
...
install_mac() {
  ...
  log "Downloading AEGIS Code..."
  local dmg="$tmp/aegis-gui.dmg"
  curl -fsSL --progress-bar "$R2/aegis-gui.dmg" -o "$dmg"
